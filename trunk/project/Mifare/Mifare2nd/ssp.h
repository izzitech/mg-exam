/****************************************************************************
 *   $Id:: ssp.h 3737 2010-06-24 02:17:30Z usb00423                         $
...
/* __SSP_DMA_TEST is a compiler option. */

#ifdef __SSP_DMA_TEST
/* If DMA is enabled above, it tests LOOPBACK mode only and use
SSP_SEL as the CS. The buffer size should be no more than half of
the SSP FIFO size. */
#define LOOPBACK_MODE	1
#define USE_CS			1
#define SSP_BUFSIZE		4

#define SSP_16BIT		1		/* default is 8-bit SSP frame size, this is to
								test 16-bit SSP frame. The DMA address needs to
								aligned completely. */
#else
/* Below are the non-DMA modes for different SSP test */
...
extern void ssp_rddata(unsigned char addr, unsigned char* data, unsigned long length);
extern void ssp_wrdata(unsigned char addr, unsigned char* data, unsigned long length);

extern void SSP_Snd_OneByte(uint8_t data);
extern unsigned char SSP_Rcv_OneByte(void);
